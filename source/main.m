...
t0 = 1;
sigma = 1;
hd = get_detector_irf(t, t0, sigma);
...
for i = 1:length(sigma_values)
    hd = get_detector_irf(t, t0_values(i), sigma_values(i));
    f = ifft(fft(x) .* fft(hf));
    d = ifft(fft(f) .* fft(hd));
    ...
    [y_n, t_n] = get_sampling(t,d,Ts);
    stem(t_n, y_n);
    title(['t0 = ', num2str(t0_values(i)), ', \sigma = ', num2str(sigma_values(i))]);
    ylabel('Amp'); grid on;
end
